...
import DataTemplates.ValidityPeriod;
...
public class MyTicketDataGenerator {
    ...
    public static ValidityPeriod generateValidPeriod(LocalDate validityDate) {
        Random random = new Random();

        int daysBackInTime = random.nextInt(45) + 3;
        int daysForwardInTime = random.nextInt(45) + 3;

        return new ValidityPeriod(validityDate.minusDays(daysBackInTime), validityDate.plusDays(daysForwardInTime));
    }
